...
public class FileUploaderController {
    ...
    /**
     * Method that performs a GET request to the endpoint localhost:8080/
    ...
    public ResponseEntity<AddressBookEntry> createFromForm(@RequestBody MultiValueMap<String, String> formData) throws ProcessingException {

        AddressBookEntry entry;
        try {
            entry = new AddressBookEntry(formData.get("first_name").get(0),
                    formData.get("last_name").get(0),
                    formData.get("street_address").get(0),
                    formData.get("additional_address").get(0),
                    formData.get("city_or_town").get(0),
                    formData.get("state").get(0),
                    formData.get("zipcode").get(0),
                    formData.get("telephone").get(0),
